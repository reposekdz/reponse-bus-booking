...
export const createCompany = asyncHandler(async (req, res: any) => {
    const company = await adminService.createCompany(req.body);
    res.status(201).json({ success: true, data: company });
});

// FIX: Removed explicit types to allow for correct type inference.
export const getCompanies = asyncHandler(async (req, res: any) => {
    const companies = await adminService.getAllCompanies();
    res.status(200).json({ success: true, count: companies.length, data: companies });
});

// FIX: Removed explicit types to allow for correct type inference.
export const updateCompany = asyncHandler(async (req, res: any) => {
    const company = await adminService.updateCompanyById(req.params.id, req.body);
    res.status(200).json({ success: true, data: company });
});

// FIX: Removed explicit types to allow for correct type inference.
export const deleteCompany = asyncHandler(async (req, res: any) => {
...
// User Controllers
// FIX: Removed explicit types to allow for correct type inference.
export const getUsers = asyncHandler(async (req, res: any) => {

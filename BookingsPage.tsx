...
import { TicketIcon, ChevronRightIcon, QrCodeIcon } from './components/icons';
            ...
            <div>
                {activeTab === 'upcoming' && (
                    <div className="space-y-4">
                        {upcomingBookings.map(booking => <BookingCard key={booking.id} booking={booking} onViewTicket={onViewTicket} />)}
                    </div>
                )}
                {activeTab === 'past' && (
                    <div className="space-y-4 opacity-75">
                        {pastBookings.map(booking => <BookingCard key={booking.id} booking={booking} onViewTicket={onViewTicket} />)}
                    </div>
                )}
            </div>
        </main>

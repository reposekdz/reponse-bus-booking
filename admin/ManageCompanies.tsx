...
const ManageCompanies: React.FC = () => {
    ...
    const handleSave = (companyData) => {
        if (currentCompany) {
            setCompanies(companies.map(c => c.id === currentCompany.id ? { ...c, ...companyData } : c));
        } else {
            const newCompany = {
                ...companyData,
                id: `company-${Date.now()}`,
                        ...
                        <tbody>
                            {companies.filter(c => c.name.toLowerCase().includes(searchTerm.toLowerCase())).map(company => (
                                <tr key={company.id} className="border-t dark:border-gray-700">
                                    ...
                                    <td>{new Intl.NumberFormat('fr-RW').format(company.totalRevenue)} RWF</td>
                                    <td>{new Intl.NumberFormat().format(company.totalPassengers)}</td>
                                    <td>{company.routesCount}</td>
                                    <td className="flex space-x-2 p-3">
                                        ...
                                        <button onClick={() => handleDelete(company.id)} className="p-1 text-gray-500 hover:text-red-600"><TrashIcon className="w-5 h-5"/></button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
